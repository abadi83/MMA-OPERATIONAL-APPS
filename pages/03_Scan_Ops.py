...
st.set_page_config(page_title="📷 Scan Ops", page_icon="📦", layout="wide")
...
# ── Scan Operational (compact version) ──
...
col3.metric("❌ Cancel", f"{c['cnt']:,}" if c else "0")
o = db.fetch_one("SELECT COUNT(DISTINCT no_pesanan) as cnt FROM penjualan")
col4.metric("📦 Orders", f"{o['cnt']:,}" if o else "0")

# Scan mode
scan_mode = st.radio("Mode", ["📦 PACK", "🚀 INSTANT", "❌ CANCEL"], horizontal=True, key="scan_ops_mode")
kategori = "BESAR" if st.checkbox("📦 Barang Besar", key="scan_ops_besar") else "REGULER"
tipe_kiriman = "INSTANT" if scan_mode == "🚀 INSTANT" else "REGULER"
...
with col_s2:
    scan_btn = st.button("📷 Scan" if scan_mode == "📦 PACK" else ("🚀 Instant" if scan_mode == "🚀 INSTANT" else "❌ Cancel"), type="primary", width="stretch")
...
if resi_input or scan_btn:
    ...
    if cleaned:
        ...
        if not cleaned:
            ...
            if dup:
                st.warning(f"🚫 Sudah di-scan: {dup['status']}")
            else:
                now = datetime.now()
                ...
                if scan_mode == "❌ CANCEL":
                    ...
                    if match:
                        ...
                        st.success(f"✅ PACKED: {real_resi} | {match['marketplace']} | {match['nama_produk'][:40]}")
                    else:
                        db.execute("INSERT INTO scan_aktif (waktu,tanggal,resi,ekspedisi,toko,status,kategori,tipe_kiriman) VALUES (?,?,?,?,?,?,?,?)",
                                   (waktu, tanggal, cleaned, "Unknown", selected_toko, "PENDING", kategori, tipe_kiriman))
                        st.warning(f"⏳ PENDING: {cleaned} - tidak ada di data penjualan")
