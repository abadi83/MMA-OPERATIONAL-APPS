...
st.set_page_config(page_title="ðŸ“¦ Purchase/OPEX", page_icon="ðŸ“¦", layout="wide")
...
tab1, tab2 = st.tabs(["ðŸ“¦ Pembelian SKU", "ðŸ’¸ Input OPEX"])

with tab1:
    st.subheader("ðŸ“¦ Pembelian SKU / Inventory")
    ...
    if st.button("ðŸ’¾ Simpan Pembelian", type="primary") and supplier and kode_sku:
        ...
        st.success(f"âœ… Pembelian {no_faktur} tersimpan! Stok +{qty}")
        st.rerun()
...
with tab2:
    ...
    if st.button("ðŸ’¾ Simpan OPEX", type="primary") and opex_desc and opex_jumlah > 0:
        tgl = datetime.now().strftime("%d-%m-%Y")
        ...
        st.success(f"âœ… OPEX {opex_desc} tersimpan!")
        st.rerun()
