...
tab1, tab2 = st.tabs(["📋 Input Resi & Pesanan", "🔄 Retur & Klaim"])

with tab1:
    st.subheader("📋 Input Resi & Pesanan")
    ...
    if st.button("💾 Simpan Pesanan", type="primary"):
        if no_pesanan and nama_produk:
            ...
            st.success(f"✅ Pesanan {no_pesanan} tersimpan!")
            st.rerun()
        else:
...
with tab2:
    ...
    if st.button("💾 Catat Retur", type="primary") and retur_resi:
